...
export const createOrder = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      console.log('heyy8orderItem',orderItem)
...
export const makePayment = async(req,res)=>{
  ...
  const orderId = uuidv4()
      ...
      const successUrl = "http://localhost:5000/payment-success"
      const failureUrl = "http://localhost:5000/payment-failure"
...
export const status = async(req,res)=>{
  ...
  const failureUrl = "http://localhost:5173/payment-success"
  const keyIndex = 1
